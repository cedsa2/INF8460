...
def read_questions(path: str) -> Tuple[List[int], List[str], List[int], List[str]]:
    ...
    if "test" not in path:
        paragraph_ids = data["paragraph_id"].fillna(-1).tolist()
...
class Preprocess(object):
    ...
    def _clean_doc(self, data):
        ...
        return [
            [
                token.lower()
                for token in tokenizer.tokenize(BeautifulSoup(review, "lxml").text)
                if len(token) > 1 # token.lower() not in self.stopwords and
                and token.isalpha()
            ]
            for review in data
        ]
